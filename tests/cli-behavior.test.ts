...
describe("CLI behavior helpers", () => {
  ...
  it("prints and writes local review CSV summaries from the CLI", () => {
    ...
    try {
      ...
      expect(JSON.parse(readFileSync(summaryPath, "utf8"))).toMatchObject({
        ...
        actionableLeads: [
          { leadKey: "url:https://fresh.test", reasons: ["unreviewed"] },
          { leadKey: "url:https://bad-date.test", reasons: ["invalid-reviewed-at"] },
          { leadKey: "url:https://lead.test", reasons: ["stale"] }
        ],
  ...
  it("filters a lead shortlist by review status", () => {
    const tmp = mkdtempSync(join(tmpdir(), "open-local-audit-cli-shortlist-review-status-"));
    try {
      const inputPath = join(tmp, "leads.csv");
      const reviewPath = join(tmp, "review.csv");
      const outPath = join(tmp, "shortlist.json");
  ...
  it("filters a lead shortlist to rows missing websites", () => {
    const tmp = mkdtempSync(join(tmpdir(), "open-local-audit-cli-shortlist-missing-website-"));
    try {
      const inputPath = join(tmp, "leads.csv");
      const outPath = join(tmp, "shortlist.json");
      writeFileSync(
      ...
      expect(JSON.parse(readFileSync(outPath, "utf8"))).toMatchObject({
        filteredRows: 1,
        selected: 1,
        leads: [{ companyName: "No Website Lead", website: "" }]
      });
    } finally {
      removeTempDir(tmp);
    }
  ...
  it("filters a lead shortlist to rows with report paths", () => {
    ...
    try {
      ...
      expect(JSON.parse(readFileSync(outPath, "utf8"))).toMatchObject({
        filteredRows: 1,
        selected: 1,
        leads: [{ companyName: "Report Lead", reportPath: "report/open-local-audit-report.html" }]
      });
    } finally {
      removeTempDir(tmp);
    }
  ...
  it("filters a lead shortlist to rows missing report paths", () => {
    const tmp = mkdtempSync(join(tmpdir(), "open-local-audit-cli-shortlist-missing-report-"));
    try {
      const inputPath = join(tmp, "leads.csv");
      const outPath = join(tmp, "shortlist.json");
      writeFileSync(
        ...
        [
          "companyName,website,priority,score,opportunityScore,topFinding,contactConfidence,reportPath",
          "Report Lead,https://report.test,high,80,92,Missing CTA,High,report/open-local-audit-report.html",
          "Missing Report Lead,https://missing.test,high,80,90,Missing CTA,High,"
        ].join("\n"),
        "utf8"
      );
      ...
      expect(JSON.parse(readFileSync(outPath, "utf8"))).toMatchObject({
        filteredRows: 1,
        selected: 1,
        leads: [{ companyName: "Missing Report Lead", reportPath: "" }]
      });
    } finally {
      removeTempDir(tmp);
    }
